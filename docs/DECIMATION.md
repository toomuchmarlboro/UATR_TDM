...
| stage | rate | length | non-zero | multiplies/output |
|---|---|---|---|---|
| 1 | 96 → 48 kHz | 39 | 21 | 11 @ 48 kHz |
| 2 | 48 → 24 kHz | 179 | 89 | 45 @ 24 kHz |
...
```
passband ripple 0-11 kHz      0.00021 dB      (limit 0.01)
droop at 11 kHz              -0.0002  dB
worst alias band             -102.0   dB      (limit -100)
...
Passband droop of 0.0002 dB means **no compensation FIR is needed**. That was
...
```
Total logic elements   5,194-5,219 / 6,272  (83%)
...
```
MAC rate      (48k x 11 + 24k x 45) x 16 ch  =  25.73 M/s
              2 multipliers x 24.576 MHz      =  1.91x headroom
multipliers   2 of 30
delay RAM     16 ch x (39 + 179) x 24 bit     =  83,712 bits = 9.1 of 30 M9K
coeff ROM     56 x 20 bit                     =  1,120 bits
accumulator   sum(|coef|) x max|data|, exact   =  45 bits
logic         one MAC + address sequencer     =  ~400-600 LE
```
