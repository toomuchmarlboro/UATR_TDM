...
def _frame():
    import re as _re
    src = open(RTL / "tdm8_master.vhd", encoding="utf-8", errors="replace").read()
...
def rtl_const(path, name, cast=int):
    src = open(RTL / path, encoding="utf-8", errors="replace").read()
...
def main():
    ...
    _lp = re.search(r'C_LR_PULSE\s*:\s*boolean\s*:=\s*(true|false)',
                    open(RTL / "tdm8_master.vhd", encoding="utf-8").read())
